"""Remove lion mask faces that are never visible once mounted on the frieze.

Same technique as scripts/strip_leafscroll_interior.py (multi-viewpoint BVH
raycast restricted to the real viewing hemisphere, early-exit per face), with
a different face axis. prep_lion() (integrate_cornice_frieze_v1.py) applies a
180 deg Z rotation before seating the mask -- world_y = -raw_y, opposite sign
from the leafscroll's 90 deg rotation -- then shifts so the back plane sits
at world y=0. That means the visible/front direction is raw -Y, and raw +Y
is flush against the wall (checked directly: a front-facing raycast sweep
found the visible surface only reaches to raw-Y +0.0001, just past the
mesh's own y=0 origin, well short of the mesh's +0.02 upper bound).

The lion's depth-density histogram does not show the leafscroll's sharp
two-hump split -- it's a thin mask shell, not a molding strip with a solid
backing mass, so the invisible fraction is expected to be much smaller.
Running the real multi-viewpoint sweep anyway rather than trusting the
single-viewpoint estimate, per the leafscroll's own lesson that a straight-on
depth measurement overestimates what a full hemisphere sweep will confirm
safe (PIPELINE-GUIDE.md SS15).

Run:  blender --background --python scripts/strip_lion_interior.py
"""
...
from paths import LION_MASTER, LION_MASTER_OBJECT, MASTERS, RENDERS  # noqa: E402

OUT_PATH = os.path.join(MASTERS, "Tripo-lion-mask-master-v5.blend")
OUT_RENDER = os.path.join(RENDERS, "lion-interior-strip")

N_CANDIDATE_DIRS = 320
MIN_NEG_Y_DOT = 0.02  # raw -Y is the face-forward axis
BACKFACE_EPS = 1e-4
HIT_DIST_TOL = 1e-4
...
def fibonacci_sphere_points(n):
    ...
    for i in range(n):
        ...
        theta = ga * i
        pts.append(Vector((math.cos(theta) * r, -y, math.sin(theta) * r)))  # -Y pole
    return pts
...
def main():
    ...
    dirs = [d for d in dirs if -d.y >= MIN_NEG_Y_DOT]
    log(f"{len(dirs)} viewpoint directions survive the -Y hemisphere filter "
        f"(dot >= {MIN_NEG_Y_DOT})")
    ...
    ob.data.name = LION_MASTER_OBJECT
    ...
    for yaw in (0, 20, 40, 60, -20, -40, -60):
        shot(f"v5_yaw{yaw}.png", yaw, 10)
    shot("v5_low.png", 0, -30)
    shot("v5_high.png", 0, 40)
